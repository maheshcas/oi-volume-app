...
def _find_oi_clusters(liquidity_map: list[dict[str, Any]], spot: float) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def compute_entry_target(context: dict) -> dict:
    ...
    try:
        ...
        band = max(float(strike_gap), resistance - support) if (support > 0 and resistance > 0) else float(strike_gap * 5)
        ...
        if trade_type == "RANGE_SELL_CE":
            ...
            stop_underlying = put_wall - round(strike_gap * 1.5, -1)
            stop_pct_premium = 0.50
            target_1 = min(call_wall, resistance - round(band * 0.3, -1))
            target_2 = call_wall
